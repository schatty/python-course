...
def check_chunk_end(acc, new_symbol):
    ...
    if new_symbol == "'" and "'" in acc:
        return True
    if new_symbol == " " and \
            ('[' not in acc and '"' not in acc and "'" not in acc):
        return True
    return False
...
def get_stats_json(df):
    ...
    stats_df = pd.concat([time_avg, time_max, time_med,
                          time_sum, time_perc], axis=1)
    stats_df.columns = ['time_avg', 'time_max',
                        'time_med', 'time_sum', 'time_perc']
...
def main():
    fn = 'logs/nginx-access-ui.log-20190630.log'

    index2props = {
            0: {'name': 'remote_addr', 'type': str},
    ...
    report_data = html_data[:insert_pos+11] + \
        str(json_dict) + html_data[insert_pos+11:]
